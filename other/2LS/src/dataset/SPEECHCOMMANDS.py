...
def _download_and_extract(root_dir):
    download_dir = os.path.dirname(root_dir) # ./data/SpeechCommands
    os.makedirs(download_dir, exist_ok=True)
    
    tar_path = os.path.join(download_dir, "speech_commands_v0.02.tar.gz")
    url = "http://download.tensorflow.org/data/speech_commands_v0.02.tar.gz"
    
    if not os.path.exists(root_dir):
        print(f"Dataset not found at {root_dir}. Downloading from {url}...")
        try:
            from tqdm import tqdm
            
            def progress_callback(count, block_size, total_size):
                if not hasattr(progress_callback, 't'):
                    progress_callback.t = tqdm(total=total_size, unit='B', unit_scale=True, desc="Downloading SpeechCommands")
                progress_callback.t.update(block_size)
                if count * block_size >= total_size:
                    progress_callback.t.close()

            urllib.request.urlretrieve(url, tar_path, reporthook=progress_callback)
            print(f"Downloaded to {tar_path}. Extracting...")
...
class SpeechCommandsDataset(Dataset):
    def __init__(self, root='./data', subset='training', n_silence=2300):
        ...
        # Pre-calculate labels for RpcClient efficiency
        self.labels = []
        for _, label in self.samples:
            if label in CLASSES:
                self.labels.append(CLASSES.index(label))
            else:
                self.labels.append(CLASSES.index('unknown'))
