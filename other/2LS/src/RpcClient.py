...
class RpcClient:
    ...
    def response_message(self, body):
        ...
        if action == "START":
            ...
            if model_name == 'VGG16':
                self.model_train = Train_VGG16(self.client_id, self.layer_id, self.channel, self.device, self.in_cluster_id, self.idx)
            elif model_name == 'BERT':
                self.model_train = Train_BERT(self.client_id, self.layer_id, self.channel, self.device, self.in_cluster_id, self.idx)
                ...
                self.model_train = Train_KWT(self.client_id, self.layer_id, self.channel, self.device, self.in_cluster_id, self.idx)
            ...
            data = {"action": "UPDATE", "client_id": self.client_id, "layer_id": self.layer_id,
                    "result": result, "size": size, "in_cluster_id": self.in_cluster_id,
