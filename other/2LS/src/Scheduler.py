...
class Scheduler:
    def __init__(self, client_id, layer_id, channel, device, in_cluster_id=-1, idx=-1):
    ...
    def send_intermediate_output(self, output, labels, trace, out_cluster_id=-1):

        forward_queue_name = f'intermediate_queue_{out_cluster_id}_{self.in_cluster_id}_{self.idx}'
    ...
    def train_on_first_layer(self, model, lr, momentum, train_loader=None, local_round=1, out_cluster_id=-1):
        ...
        for i in range(local_round):
            ...
            src.Log.print_with_color(f'Forward epoch {i+1}/{local_round}', 'green')
            ...
            with tqdm(total=len(train_loader), desc="Processing", unit="step") as pbar:
                while True:
                    try:
                        ...
                        self.send_intermediate_output(intermediate_output, labels, trace=None, out_cluster_id=out_cluster_id)
                        while True:
                            model.train()
                            optimizer.zero_grad()
                            ...
                            if method_frame and body:
                                ...
                                # Check for PAUSE from server (Early Termination)
                                broadcast_queue_name = f'reply_{self.client_id}'
                                method_f, header_f, body_f = self.channel.basic_get(queue=broadcast_queue_name, auto_ack=True)
                                if body_f:
                                    received_data = pickle.loads(body_f)
                                    src.Log.print_with_color(f"[<<<] Received message during gradient wait: {received_data}", "blue")
                                    if received_data.get("action") == "PAUSE":
                                        return True
                                time.sleep(0.5)
                                continue
    ...
    def train_on_last_layer(self, model, lr, momentum, out_cluster_id=-1):
        ...
        while True:
            ...
            if method_frame and body:
                ...
                if body_f:
                    received_data = pickle.loads(body_f)
                    src.Log.print_with_color(f"[<<<] Received message during idle wait: {received_data}", "blue")
                    if received_data.get("action") == "PAUSE":
                        return True
