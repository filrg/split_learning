...
def compute_mfcc(waveform, sample_rate=16000, n_mfcc=40, n_fft=480, hop_length=160, n_mels=40):
    ...
    return mfcc.T  # (n_mfcc, time_steps)
...
class SpeechCommandsDataset(Dataset):
    ...
    def __init__(self, root='./data', subset='training', n_silence=2300):
        ...
        # Collect keyword & unknown samples based on subset
        ...
        # Generate silence samples (random 1s crops from background noise)
        # Following the paper: silence class created from _background_noise_
        if self.noise_data:
            if subset == 'training':
                num_silence = n_silence
            else:
                num_silence = max(1, n_silence // 9)  # ~260 for val/test
            
            for _ in range(num_silence):
                # path=None marks this as a silence sample (generated on-the-fly)
    ...
    def _get_silence_waveform(self):
        """Generate a 1-second silence waveform by random crop from background noise"""
        target_length = 16000
        noise = random.choice(self.noise_data)
        
        if len(noise) <= target_length:
            # Noise file too short, pad with zeros
    ...
    def __getitem__(self, idx):
        ...
        try:
            if audio_path is None:
                # Silence sample: random crop from background noise
