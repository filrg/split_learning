    ...
    def on_request(self, ch, method, props, body):
        ...
        if action == "REGISTER":
            ...
            if (str(client_id), layer_id) not in self.list_clients:
                ...
                if layer_id == 1:
                    self.edge_device.append((str(client_id), layer_id))
                else:
                    self.device_begin.append((str(client_id), layer_id))
                    self.device_stop.append((str(client_id), layer_id))
            ...
            if self.idx < self.total_clients[0] - 1:
                (client_id, layer_id) = self.edge_device[self.idx]
                self.send_to_response(client_id, pickle.dumps(message))
            else:
                for (client_id, layer_id) in self.device_stop:
                    self.send_to_response(client_id, pickle.dumps(message))
    ...
    def notify_clients(self, start=True, register=True, idx = 0):
        if start:
            if register:
                ...
                for (client_id, layer_id) in self.device_begin:
                ...
                label = self.label_counts[idx]
                layers = [0, self.list_cut_layers[0]]
                (client_id, layer_id) = self.edge_device[idx]
                response = {"action": "START",
                            "message": "Server accept the connection!",
