...
class Server:
    def __init__(self, config):
        ...
        self.connection = pika.BlockingConnection(
            pika.ConnectionParameters(host=address, port=5672, virtual_host=f'{virtual_host}', credentials=credentials))
    ...
    def on_request(self, ch, method, props, body):
        ...
        if action == "REGISTER":
            ...
            if (str(client_id), layer_id, cluster) not in self.list_clients:
                self.list_clients.append((str(client_id), layer_id, cluster))
                if layer_id == 1:
                    self.edge_device.append((str(client_id), layer_id, cluster))
                    self.info_cluster[cluster] += 1
                else:
                    self.device_begin.append((str(client_id), layer_id, cluster))
                    self.device_stop.append((str(client_id), layer_id, cluster))
            ...
            if self.register_clients == self.total_clients:
                for (client_id, layer_id, cluster) in self.edge_device:
                    if cluster == 0:
                        self.device_begin.append((client_id, layer_id, cluster))
                    elif cluster == self.num_cluster -1:
                        self.device_stop.append((client_id, layer_id, cluster))
                self.distribution()
    ...
    def notify_clients(self, start=True, register=True, idx = 0):
        if start:
            ...
            for (client_id, layer_id, cluster) in self.list_clients:
