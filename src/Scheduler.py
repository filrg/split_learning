...
import src.Log
from src.Detector import ZPerLabelRuleDetectorOnline
from src.Store import ZLogger
...
class Scheduler:
    ...
    def send_intermediate_output(self, data_id, label_count, output, labels, trace, test=False, cluster=None,
                                 special=False):
        if special is True:
        ...
        if trace:
            ...
            message = pickle.dumps(
                {"data_id": data_id, "label_count": label_count, "data": output.detach().cpu().numpy(), "label": labels,
                 "trace": trace,
                 "test": test}
            )
        else:
    ...
    def train_on_first_layer(self, model, global_model, label_count, lr, momentum, clip_grad_norm, compute_loss,
                             control_count=5,
                ...
                if method_frame and body:
                    ...
                    try:
                        ...
                        self.send_intermediate_output(data_id, label_count, intermediate_output, labels, trace=None,
                                                      test=False, cluster=cluster, special=special)
    ...
    def train_on_last_layer(self, model, global_model, label_count, lr, momentum, clip_grad_norm, compute_loss, cluster,
        ...
        while True:
            ...
            if method_frame and body:
                ...
                # self.all_forward.append({'client': trace[-1], 'data': intermediate_output_numpy})
    ...
    def train_on_device(self, model, global_model, label_count, lr, momentum, clip_grad_norm, compute_loss, num_layers,
                        control_count, train_loader=None, cluster=None, special=False, alone_train=False):
        self.data_count = 0
        if self.layer_id == 1:
        ...
        #     with open(f"storage/save_forward_{self.round}.pkl", "wb") as f:
        #         pickle.dump(self.all_forward, f)
        self.round += 1
        self.all_backward = []
        self.all_forward = []
