...
class Server_cluster:
    def __init__(self, config):
        ...
        self.num_cluster = self.partition["num-cluster"]
        self.info_cluster = [0 for _ in range(self.num_cluster)]
    ...
    def on_request(self, ch, method, props, body):
        ...
        if action == "REGISTER":
            ...
            if (str(client_id), layer_id, performance, 0) not in self.list_clients:
                self.list_clients.append((str(client_id), layer_id, performance, performance))
                if layer_id == 1:
                    self.edge_device.append((str(client_id), layer_id, performance, performance))
                    self.info_cluster[performance] += 1
                else:
                    self.device_begin.append((str(client_id), layer_id, performance, performance))
                    self.device_stop.append((str(client_id), layer_id, performance, performance))
            ...
            if self.register_clients == self.total_clients:
                for (client_id, layer_id, performance, clustering) in self.edge_device:
                    if clustering == 0:
                        self.device_begin.append((client_id, layer_id, performance, clustering))
                    elif clustering == self.num_cluster -1:
                        self.device_stop.append((client_id, layer_id, performance, clustering))
                self.distribution()
            ...
            if self.idx < self.num_cluster - 1:
                ...
                if self.current_clients_cluster == self.info_cluster[self.idx]:
                    self.current_clients_cluster = 0
                    for (client_id, layer_id, _, clustering) in self.edge_device:
                        if clustering == self.idx:
